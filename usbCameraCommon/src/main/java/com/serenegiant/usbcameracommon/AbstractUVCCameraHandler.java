...
abstract class AbstractUVCCameraHandler extends Handler {
	...
	private static final int MSG_MEDIA_UPDATE_VIDEO = 7;
	private static final int MSG_MEDIA_UPDATE_IMAGE = 13;
	private static final int MSG_RELEASE = 9;
	private static final int MSG_CAPTURE_START_WITH_PATH_AND_NAME = 10;
	private static final int MSG_CAPTURE_START_WITH_FILE = 11;
	private static final int MSG_CAPTURE_STOP_WITH_PATH = 12;
	private static final int MSG_SET_AUTO_FOCUS = 20;
	...
	public void startRecording(String path, String name) {
		checkReleased();
		String[] strings = {path, name};
		sendMessage(obtainMessage(MSG_CAPTURE_START_WITH_PATH_AND_NAME, strings));
	}

	public void startRecording(File file) {
		checkReleased();
		sendMessage(obtainMessage(MSG_CAPTURE_START_WITH_FILE, file));
	}
	...
	public void stopRecording(String path) {
		sendMessage(obtainMessage(MSG_CAPTURE_STOP_WITH_PATH, path));
	}
	...
	protected void updateMedia(final String path) {
		sendMessage(obtainMessage(MSG_MEDIA_UPDATE_IMAGE, path));
	}
	...
	public void handleMessage(final Message msg) {
		...
		case MSG_CAPTURE_START_WITH_PATH_AND_NAME:
			thread.handleStartRecording((String[])msg.obj);
			break;
		case MSG_CAPTURE_START_WITH_FILE:
			thread.handleStartRecording((File) msg.obj);
			break;
		case MSG_CAPTURE_STOP:
			thread.handleStopRecording();
			break;
		case MSG_CAPTURE_STOP_WITH_PATH:
			thread.handleStopRecording((String)msg.obj);
			break;
		case MSG_MEDIA_UPDATE_VIDEO:
			thread.handleUpdateMedia((String)msg.obj);
			break;
		case MSG_MEDIA_UPDATE_IMAGE:
			thread.handleUpdateMediaNoCallback((String)msg.obj);
			break;
		case MSG_RELEASE:
			thread.handleRelease();
			break;
		case MSG_SET_AUTO_FOCUS:
...
//						bitmap.compress(Bitmap.CompressFormat.PNG, 100, os);
						...
						os.flush();
						mHandler.sendMessage(mHandler.obtainMessage(MSG_MEDIA_UPDATE_IMAGE, outputFile.getPath()));
		...
		public void handleStartRecording(String[] strings) {
			if (DEBUG) Log.v(TAG_THREAD, "handleStartRecording:\npath:" + strings[0] + "\nname:" + strings[1]);
			try {
				if ((mUVCCamera == null) || (mMuxer != null)) return;
				final MediaMuxerWrapper muxer = new MediaMuxerWrapper(strings[0], strings[1], ".mp4");	// if you record audio only, ".m4a" is also OK.
				muxerStartRecording(muxer);
			} catch (final IOException e) {
				callOnError(e);
				Log.e(TAG, "startCapture:2", e);
			}
		}

		public void handleStartRecording(File file) {
			if (DEBUG) Log.v(TAG_THREAD, "handleStartRecording:\nfile:" + file.getAbsolutePath());
			try {
				if ((mUVCCamera == null) || (mMuxer != null)) return;
				final MediaMuxerWrapper muxer = new MediaMuxerWrapper(file);	// if you record audio only, ".m4a" is also OK.
				muxerStartRecording(muxer);
			} catch (final IOException e) {
				callOnError(e);
				Log.e(TAG, "startCapture:3", e);
			}
		}

		private void muxerStartRecording(final MediaMuxerWrapper muxer) throws IOException {
		...
		public void handleStopRecording(String path) {
			if (DEBUG) Log.v(TAG_THREAD, "handleStopRecording:mMuxer=" + mMuxer);
			final MediaMuxerWrapper muxer;
			synchronized (mSync) {
				muxer = mMuxer;
		...
		public void handleUpdateMediaNoCallback(final String path) {
			...
			public void onStopped(final MediaEncoder encoder) {
				...
				try {
					...
					if (!TextUtils.isEmpty(path)) {
						mHandler.sendMessageDelayed(mHandler.obtainMessage(MSG_MEDIA_UPDATE_VIDEO, path), 1000);
					} else {
						final boolean released = (mHandler == null) || mHandler.mReleased;
		...
		private void callOnCaptureStill() {
			for (final CameraCallback callback: mCallbacks) {
				try {
					callback.onCaptureStill();
				} catch (final Exception e) {
					mCallbacks.remove(callback);
					Log.w(TAG, e);
				}
		...
		private void callOnUpdateMedia() {
			for (final CameraCallback callback: mCallbacks) {
				try {
					callback.onUpdateMedia();
				} catch (final Exception e) {
					mCallbacks.remove(callback);
					Log.w(TAG, e);
				}
	...
	public void setAutoFocus(Boolean autoFocus) {
		checkReleased();
		sendMessage(obtainMessage(MSG_SET_AUTO_FOCUS, autoFocus));
	}
