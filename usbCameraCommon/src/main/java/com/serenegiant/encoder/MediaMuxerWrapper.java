...
public class MediaMuxerWrapper {
	...
	private static final String DIR_NAME = "/FSON";//"/$MuMu共享文件夹/FSON";
	 ...
	 * @throws IOException
	 */
	public MediaMuxerWrapper(String path, String name, String ext) throws IOException {
		if (TextUtils.isEmpty(ext)) ext = ".mp4";
		try {
			mOutputPath = getCaptureFile(Environment.getExternalStorageDirectory().getAbsolutePath() + path, "", ext).toString();
		} catch (final NullPointerException e) {
			throw new RuntimeException("This app has no permission of writing external storage");
		}
		mMediaMuxer = new MediaMuxer(mOutputPath, MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);
	 ...
	 * @throws IOException
	 */
	public MediaMuxerWrapper(File file) throws IOException {
		if (DEBUG) Log.v(TAG,  file.getAbsolutePath());
		mMediaMuxer = new MediaMuxer(file.getAbsolutePath(), MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);
		if (DEBUG) Log.v(TAG,  "MediaMuxerWrapper:完成");
		mEncoderCount = mStatredCount = 0;
		mIsStarted = false;
	}
...
//**********************************************************************
	...
	/**
	 * generate output file
	 * @param path Environment.getExternalStorageDirectory() etc.
	 * @param name name of output file
	 * @param ext .mp4(.m4a for audio) or .png
	 * @return return null when this app has no writing permission to external storage.
	 */
	public static final File getCaptureFile(final String path, final String name, final String ext) {
		final File dir = new File(path, DIR_NAME);
		Log.d(TAG, "path=" + dir.toString());
		dir.mkdirs();
		if (dir.canWrite()) {
